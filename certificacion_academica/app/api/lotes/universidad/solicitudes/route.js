...
import { listarSolicitudesLotesUniversidadOnchain } from "../../../../lib/onchain/registro";

export async function GET(request) {
  const { searchParams } = new URL(request.url);
  const walletUniversidad = String(searchParams.get("walletUniversidad") || "").trim();
  try {
    const resultado = await listarSolicitudesLotesUniversidadOnchain(walletUniversidad);
